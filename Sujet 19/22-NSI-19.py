def multiplication(n1:int, n2:int)->int:
    """renvoie le produit de n1 et n2"""
    assert type(n1) == type(n2) == int, "n1 et n2 doivent etre des entiers"
    if n2 == 0: # 0 * n1 = 0 dans tous les cas
        return 0
    elif n2 > 0: # si n2 est positif on ajoute n1 et le produit de n1 et (n2 - 1)
...
def chercher(T,n,i,j):
    ...
    if T[m] < n : # si la valeur du milieu est plus petite que n
        return chercher(T, n, m + 1 , j) # le debut devient le milieu
    elif T[m] > n : # sinon si elle est plus grande que n
        return chercher(T, n, i , j - 1 ) # la fin devient le milieu
    else :
        return m # sinon on a l'indice
